...
def main():
    ...
    # メインエリア
    main_tab, *law_tabs = st.tabs(["チャット"] + list(st.session_state.selected_laws.keys()))
    ...
    # 法令タブの表示
    for law_name, law_tab in zip(st.session_state.selected_laws.keys(), law_tabs):
        with law_tab:
            st.subheader(f"法令全文: {law_name}")
            st.text_area("", st.session_state.selected_laws[law_name], height=600)
